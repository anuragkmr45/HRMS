...
export class AttendanceCommandService {
  ...
  async execute(input: {
    ...
    try {
      const result = await repository.transaction<AttendanceCommandOutcome>(async (tx) => {
        ...
        if (!stateDecision.allowed || policyReason || (open?.last_transition_at && Date.parse(occurredAt) < Date.parse(open.last_transition_at))) {
          const reason = !stateDecision.allowed ? stateDecision.reason_detail : policyReason ?? "Client timestamp precedes the previous session transition.";
          const code = !stateDecision.allowed ? stateDecision.reason_code : policyReason ? "policy_window_rejected" : "invalid_chronology";
        ...
        try {
          session = await this.transition(tx, input.companyId, input.actor.id, workDate, occurredAt, input.command, open, stateDecision.action);
        } catch (error) {
          if (isUniqueViolation(error)) throw conflict("The employee already has an open attendance session.");
          throw error;
        }
        await tx.updateEmployeeState({ companyId: input.companyId, employeeUserId: input.actor.id, state: stateDecision.next_state, currentSessionId: stateDecision.next_state === "not_checked_in" ? null : session.id });
        const punch = (await tx.insertPunchEvent({ companyId: input.companyId, employeeUserId: input.actor.id, eventType: input.command.event_type, occurredAt, workMode: input.command.work_mode, source: input.command.source, metadata: input.command.metadata, commandExecutionId: command.id, sessionId: session.id, decisionId: decision.id })).rows[0]!;
        const day = await projectDay(tx, input.companyId, input.actor.id, session.work_date, input.command.work_mode, input.policy.graceMinutes, input.timeZone);
  ...
  private async transition(tx: AttendanceCommandTransactionRepository, companyId: UUID, employeeId: UUID, workDate: string, occurredAt: string, command: AttendanceCommandInput, open: AttendanceSessionRecord | null, action: string): Promise<AttendanceSessionRecord> {
    if (action === "open_session") return tx.createSession({ companyId, employeeUserId: employeeId, workDate, checkedInAt: occurredAt, workMode: command.work_mode, source: command.source, metadata: command.metadata });
    if (!open) throw conflict("There is no open attendance session.");
    const args = { sessionId: open.id, companyId, employeeUserId: employeeId, expectedVersion: open.version, occurredAt };
...
function policyBlocked(type: AttendancePunchEventType, at: string, zone: string, policy: CommandPolicy, working: boolean, state: string): string | null { const local = new Intl.DateTimeFormat("en-GB", { timeZone: zone, hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(new Date(at)); if (type === "check_in" && state === "not_checked_in" && !policy.allowOffDayPunches && !working) return "Punch-in is not allowed on company off days."; if (policy.fullDayPunchWindow) return null; const range: [string, string] | null = type === "check_in" ? [policy.punchInStart, policy.punchInEnd] : type === "check_out" ? [policy.punchOutStart, policy.punchOutEnd] : null; if (!range) return null; const now = toMinutes(local), start = toMinutes(range[0]), end = toMinutes(range[1]); if ((start <= end && (now < start || now > end)) || (start > end && now > end && now < start)) return `Punch-${type === "check_in" ? "in" : "out"} is allowed between ${range[0]} and ${range[1]}.`; return null; }
function toMinutes(value: string): number { const [h = 0, m = 0] = value.split(":").map(Number); return h * 60 + m; }
function dateInTimeZone(value: string, timeZone: string): string { const parts = new Intl.DateTimeFormat("en-CA", { timeZone, year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(new Date(value)); const data = Object.fromEntries(parts.map((p) => [p.type, p.value])); return `${data.year}-${data.month}-${data.day}`; }
async function projectDay(tx: AttendanceCommandTransactionRepository, companyId: UUID, employeeId: UUID, workDate: string, workMode: string, grace: number, timeZone: string): Promise<Record<string, unknown>> { const punches = (await tx.query<PunchProjectionRow>(`SELECT event_type, occurred_at, work_mode FROM attendance.punch_events WHERE company_id=$1 AND employee_user_id=$2 AND deleted_at IS NULL AND occurred_at >= $3::date AND occurred_at < ($3::date + interval '2 days') ORDER BY occurred_at`, [companyId, employeeId, workDate])).rows; const checkIn = punches.find((p) => p.event_type === "check_in")?.occurred_at ?? null; const checkOut = punches.filter((p) => p.event_type === "check_out").at(-1)?.occurred_at ?? null; let breakStart: string | null = null, breaks = 0; for (const p of punches) { if (p.event_type === "break_start") { breakStart = p.occurred_at; } else if (p.event_type === "break_end" && breakStart !== null) { const breakStartedAt = breakStart; breaks += Math.max(0, Math.round((Date.parse(p.occurred_at) - Date.parse(breakStartedAt)) / 60000)); breakStart = null; } } const work = checkIn ? Math.max(0, Math.round(((checkOut ? Date.parse(checkOut) : Date.now()) - Date.parse(checkIn)) / 60000) - breaks) : 0; const status = workMode === "wfh" ? AttendanceDayStatuses.Wfh : AttendanceDayStatuses.Present; const row = (await tx.query<Record<string, unknown>>(`INSERT INTO attendance.daily_records (company_id,employee_user_id,work_date,status,first_check_in,last_check_out,work_minutes,break_minutes,late_minutes,early_out_minutes,work_mode,note,exception_type,regularization_status,version,created_at,updated_at,deleted_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,0,0,$9,NULL,NULL,NULL,1,now(),now(),NULL) ON CONFLICT (company_id,employee_user_id,work_date) DO UPDATE SET status=EXCLUDED.status,first_check_in=EXCLUDED.first_check_in,last_check_out=EXCLUDED.last_check_out,work_minutes=EXCLUDED.work_minutes,break_minutes=EXCLUDED.break_minutes,work_mode=EXCLUDED.work_mode,version=attendance.daily_records.version+1,updated_at=now() RETURNING *`, [companyId, employeeId, workDate, status, checkIn, checkOut, work, breaks, workMode])).rows[0]; return row ?? {}; }
