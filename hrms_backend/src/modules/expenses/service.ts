...
import {
  assertTransition,
  financeDecisionToStatus,
  isTerminalExpenseStatus,
  managerDecisionToStatus,
...
export class ExpenseService {
  ...
  createTicket(actor: AuthUser, input: ExpenseCreateInput): ExpenseTicket {
    ...
    const route = input.submit
      ? this.resolveRoute(
          actor.id,
          input.expense_type,
          input.expense_sub_type,
          input.payment_type,
        )
      : null;
    ...
    const ticket: ExpenseTicket = {
      ...
      policy_snapshot: {
        required_documents:
          RequiredDocumentsByExpenseSubType[input.expense_sub_type],
      },
      version: 1,
      created_at: created,
      updated_at: created,
  ...
  submitTicket(
    ...
    if (!resubmittableStatuses.includes(current.status)) {
      throw badRequest(
        "Only draft or manager-returned tickets can be submitted",
      );
    }
    const route = this.resolveRoute(
      current.requester_user_id,
      current.expense_type,
      current.expense_sub_type,
      current.payment_type,
    );
    assertTransition(current.status, ExpenseStatuses.Submitted);
    const ticket = this.repository.updateTicketVersioned(
      ticketId,
      expectedVersion,
      (candidate) => {
  ...
  private validateCreateInput(input: ExpenseCreateInput): void {
    ...
    if (
      input.expense_type === ExpenseTypes.Project &&
      !(ProjectExpenseSubTypes as readonly string[]).includes(
        input.expense_sub_type,
      )
    ) {
    ...
    if (
      input.expense_type === ExpenseTypes.SalesPreSales &&
      !(SalesExpenseSubTypes as readonly string[]).includes(
        input.expense_sub_type,
      )
    ) {
    ...
    if (input.payment_type === PaymentTypes.Advance) {
      ...
      if (
        compareMoney(input.advance_amount, input.estimated_amount) > 0 &&
        !input.advance_justification
      ) {
        throw badRequest(
          "Advance cannot exceed estimated amount unless exception remarks exist",
        );
      }
    }
  }
  ...
  resolveRoute(
    ...
    if (
      requester.employment_status !== EmploymentStatuses.Active ||
      requester.deleted_at
    ) {
      throw badRequest(
        "Inactive or soft-deleted users cannot submit expense tickets",
      );
    }
    ...
    if (!managerResolution.assignedManagerId) {
      throw badRequest(
        "Expense cannot be submitted until an independent Manager approver is configured.",
        {
          requester_user_id: requesterUserId,
          notes,
          required_setup:
            "Assign an active non-requester Manager or Manager backup before submission.",
        },
      );
    }
    if (!financeResolution.assignedFinanceActorId) {
      throw badRequest(
        "Expense cannot be submitted until an independent Finance Manager approver is configured.",
        {
          ...
          notes,
          required_setup:
            "Configure an active non-requester Finance Manager or Finance Manager backup before submission.",
        },
      );
    }
    return {
      ...
      routeSnapshot: {
        ...
        primary_finance_manager_user_id:
          financeResolution.primaryFinanceManagerId,
  ...
  private resolveManagerActor(requesterUserId: UUID): ManagerRoutingResolution {
    ...
    const directManager = requester.manager_user_id
      ? (this.store.users.find(
          (user) => user.id === requester.manager_user_id && !user.deleted_at,
        ) ?? null)
      : null;
    if (
      directManager &&
    ...
    const backupAssignment = this.repository.activeManagerBackupAssignment(
      requesterUserId,
      today,
    );
    const backupId =
      backupAssignment?.backup_manager_user_id ??
      this.store.financeGovernanceConfig?.manager_backup_user_id ??
      null;
    if (!backupId) {
      return {
        ...
        backupManagerId: null,
        backupApplied: false,
        notes: [...notes, "manager_backup_missing"],
      };
    }
    if (backupId === requesterUserId) {
      return {
        assignedManagerId: null,
        backupManagerId: backupId,
        backupApplied: false,
        notes: [...notes, "manager_backup_self_reference"],
      };
    }
    const backup =
      this.store.users.find(
        (user) => user.id === backupId && !user.deleted_at,
      ) ?? null;
    if (
      !backup ||
      backup.employment_status !== EmploymentStatuses.Active ||
      !isManagerRole(backup)
    ) {
      return {
        assignedManagerId: null,
        backupManagerId: backupId,
        backupApplied: false,
        notes: [...notes, "manager_backup_invalid"],
      };
    }
    return {
      assignedManagerId: backup.id,
      backupManagerId: backup.id,
      backupApplied: true,
      notes: [...notes, "manager_backup_applied"],
    };
  }
  ...
  private resolveFinanceActor(requesterUserId: UUID): FinanceRoutingResolution {
    ...
    if (!config || config.deleted_at || config.status !== "active") {
      return {
        ...
        primaryFinanceManagerId:
          config?.primary_finance_manager_user_id ?? null,
    ...
    if (
      ...
      (config.effective_to && config.effective_to < today)
    ) {
      return {
        assignedFinanceActorId: null,
    ...
    const primary =
      this.store.users.find(
        (user) =>
          user.id === config.primary_finance_manager_user_id &&
          !user.deleted_at,
      ) ?? null;
    if (
      !primary ||
      primary.employment_status !== EmploymentStatuses.Active ||
      !primary.roles.includes(Roles.FinanceManager)
    ) {
      return {
        assignedFinanceActorId: null,
        primaryFinanceManagerId: config.primary_finance_manager_user_id,
  ...
  listMyTickets(
    ...
    return this.presentPaginatedTickets(
      this.repository.listByRequester(actor.id),
      page,
      pageSize,
    );
  }
  ...
  metadata(actor: AuthUser): Record<string, unknown> {
    const activeProjects = this.store.projects
      ...
      .map((project) => ({
        ...
        manager_name:
          this.store.users.find((user) => user.id === project.manager_user_id)
            ?.full_name ?? null,
      }));
    const activeClients = Array.from(
      new Set(activeProjects.map((project) => project.client).filter(Boolean)),
    ).sort();
    return {
      generated_at: nowIso(),
      actor_scope: {
      ...
      expense_sub_types: [
        ...ProjectExpenseSubTypes.map((value) => ({
          value,
          label: value,
          expense_type: ExpenseTypes.Project,
        })),
        ...SalesExpenseSubTypes.map((value) => ({
          value,
          label: value,
          expense_type: ExpenseTypes.SalesPreSales,
        })),
      ],
      project_expense_types: [
        {
          value: "travel",
          label: "Travel",
          expense_sub_type: ExpenseSubTypes.ProjectTravel,
        },
        {
          value: "material",
          label: "Material",
          expense_sub_type: ExpenseSubTypes.MaterialConsumables,
        },
        {
          value: "lodging",
          label: "Lodging & Boarding",
          expense_sub_type: ExpenseSubTypes.LodgingBoarding,
        },
        {
          value: "misc",
          label: "Miscellaneous",
      ...
      document_types: Object.entries(RequiredDocumentsByExpenseSubType).map(
        ([expense_sub_type, required_documents]) => ({
          expense_sub_type,
          required_documents,
        }),
      ),
  ...
  dashboardSummary(
    ...
    const mine = visible.filter(
      (ticket) => ticket.requester_user_id === actor.id,
    );
    const managerQueue = visible.filter(
      (ticket) =>
    ...
    const submittedStatuses: readonly ExpenseStatus[] = [
    ...
    const pendingApprovalStatuses: readonly ExpenseStatus[] = [
      ExpenseStatuses.PendingManagerVerification,
      ExpenseStatuses.ManagerVerified,
    ];
    ...
    const returnedOrRejectedStatuses: readonly ExpenseStatus[] = [
      ExpenseStatuses.ManagerReturned,
    ...
    const financeQueue = visible.filter(
      (ticket) =>
        ticket.finance_approver_id === actor.id &&
        financeStatuses.includes(ticket.status),
    );
    const totals = {
      submitted: mine.filter((ticket) =>
        submittedStatuses.includes(ticket.status),
      ).length,
      pending_approval: mine.filter((ticket) =>
        pendingApprovalStatuses.includes(ticket.status),
      ).length,
      approved: mine.filter((ticket) =>
        approvedStatuses.includes(ticket.status),
      ).length,
      returned_or_rejected: mine.filter((ticket) =>
        returnedOrRejectedStatuses.includes(ticket.status),
      ).length,
      finance_pending: financeQueue.filter(
        (ticket) => ticket.status === ExpenseStatuses.ManagerVerified,
      ).length,
      payment_pending: financeQueue.filter(
        (ticket) => ticket.status === ExpenseStatuses.FinanceApproved,
      ).length,
      settlement_pending: visible.filter((ticket) =>
        settlementStatuses.includes(ticket.status),
      ).length,
    };
    return {
      generated_at: nowIso(),
      ...
      cards: [
        ...
        },
        {
          ...
          value: totals.pending_approval,
          tone: "warning",
        },
        {
          key: "finance_pending",
          label: "Finance pending",
          value: totals.finance_pending,
          tone: "info",
        },
        {
          key: "payment_pending",
          label: "Payment pending",
          value: totals.payment_pending,
          amount: this.sumTickets(
            financeQueue.filter(
              (ticket) => ticket.status === ExpenseStatuses.FinanceApproved,
            ),
          ),
        ...
        },
        {
          ...
          value: totals.settlement_pending,
          tone: "warning",
        },
        {
          key: "returned_or_rejected",
          label: "Returned / Rejected",
          value: totals.returned_or_rejected,
          tone: "destructive",
        },
      ],
      queue_counts: {
  ...
  withdrawTicket(
    ...
    if (!allowedStatuses.includes(current.status)) {
      throw badRequest(
        "Only draft, submitted, pending-manager, or manager-returned tickets can be withdrawn",
        {
    ...
    if (current.status !== ExpenseStatuses.Draft && !remarks?.trim()) {
      throw badRequest(
        "Remarks are required to withdraw a submitted expense ticket",
      );
    }
    ...
    const timelineEvent = this.writeMutation(
      actor,
      ticket,
      "expense.withdrawn",
      previousStatus,
    ...
    this.addNotification(
      actor.id,
      ticket.manager_verifier_id ?? ticket.finance_approver_id,
      "expense.withdrawn",
      { ticket_id: ticket.id },
    );
    return {
  ...
  addClarification(
    actor: AuthUser,
    ticketId: UUID,
    input: ExpenseClarificationInput,
  ): Record<string, unknown> {
    const ticket = this.repository.findTicket(ticketId);
    if (!canReadTicket(actor, ticket)) {
      throw forbidden(
        "You do not have access to this ticket clarification thread",
      );
    }
    if (isTerminalExpenseStatus(ticket.status)) {
      throw badRequest("Closed, cancelled, or rejected tickets are read-only", {
        current_status: ticket.status,
      });
    }
    if (
      input.expected_version !== undefined &&
      ticket.version !== input.expected_version
    ) {
      throw conflict(
        "The ticket was modified by another actor. Refresh and retry.",
        {
    ...
    this.addNotification(actor.id, target, "expense.clarification_added", {
      ticket_id: ticket.id,
    });
    return {
  ...
  financeQueue(
    ...
    const isConfiguredPrimaryFinanceManager =
      ...
      this.store.financeGovernanceConfig?.primary_finance_manager_user_id ===
        actor.id &&
    ...
    if (
      !canSeeAllFinance &&
      !isConfiguredPrimaryFinanceManager &&
      !hasAssignedFinanceWork
    ) {
      throw forbidden(
        "Only the assigned finance actor, Finance Manager, Auditor, or Admin can read the finance queue",
      );
    }
    ...
    const filtered = this.store.tickets
      ...
      .filter(
        (ticket) => canSeeAllFinance || ticket.finance_approver_id === actor.id,
      )
      .filter((ticket) => statusScope.includes(ticket.status))
      .filter(
        (ticket) =>
          !filters.payment_type || ticket.payment_type === filters.payment_type,
      )
      .filter(
        (ticket) =>
          !filters.expense_type || ticket.expense_type === filters.expense_type,
      )
      .filter(
        (ticket) =>
          !filters.expense_sub_type ||
          ticket.expense_sub_type === filters.expense_sub_type,
      )
      .filter(
        (ticket) =>
          !filters.date_from ||
          ticket.created_at.slice(0, 10) >= filters.date_from,
      )
      .filter(
        (ticket) =>
          !filters.date_to || ticket.created_at.slice(0, 10) <= filters.date_to,
      )
      .filter(
        (ticket) =>
          !filters.amount_min ||
          compareMoney(ticket.estimated_amount, filters.amount_min) >= 0,
      )
      .filter(
        (ticket) =>
          !filters.amount_max ||
          compareMoney(ticket.estimated_amount, filters.amount_max) <= 0,
      )
      .filter((ticket) => {
        if (!requesterFilter) {
          return true;
        ...
        const requester = this.store.users.find(
          (user) => user.id === ticket.requester_user_id,
        );
        return `${requester?.employee_code ?? ""} ${requester?.full_name ?? ""}`
          .toLowerCase()
          .includes(requesterFilter);
      })
      .filter((ticket) => {
        if (!departmentFilter) {
          return true;
        ...
        const department = this.store.departments.find(
          (candidate) => candidate.id === ticket.department_id,
        );
      ...
      .filter((ticket) => {
        ...
        return filters.document_status === "missing"
          ? missing.length > 0
          : missing.length === 0;
      })
      .filter((ticket) => {
      ...
      .sort((left, right) =>
        this.compareFinanceQueueTickets(left, right, filters.sort),
      );
  ...
  financeDetail(actor: AuthUser, ticketId: UUID): Record<string, unknown> {
    ...
    return {
      ...
      documents: this.store.expenseDocuments.filter(
        (document) => document.ticket_id === ticket.id,
      ),
      payments,
      ...
      policy_warnings: [
        ...
        ...(ticket.requester_user_id === actor.id
          ? [{ code: "self_processing_blocked", severity: "must" }]
          : []),
      ],
      settlement: {
  ...
  financeApprove(
    ...
    const financeRemarkDecisions: readonly string[] = [
  ...
  releasePayment(
    ...
    this.addNotification(
      ...
      ticket.requester_user_id,
      "expense.payment_released",
      { ticket_id: ticket.id, reference_no: input.reference_no },
    );
    return ticket;
  }
  ...
  submitBills(
    ...
    const previousStatus = current.status;
    if (
      current.requester_user_id !== actor.id &&
      !actor.roles.includes(Roles.FinanceManager) &&
  ...
  settle(
    ...
    if (!settlementReadyStatuses.includes(current.status)) {
      throw badRequest(
        "Settlement requires Payment Released, Bills Submitted, or Pending Adjustment status",
      );
    }
    const missing = this.missingRequiredDocuments(current);
    if (missing.length > 0) {
      throw badRequest(
        "Closure blocked until required documents are uploaded and verified by the assigned manager",
        {
    ...
    const ticket = this.repository.updateTicketVersioned(
      ...
      (candidate) => {
        ...
        candidate.closed_at =
          nextStatus === ExpenseStatuses.Closed ? nowIso() : null;
      },
    );
    const payment = this.store.payments.find(
      (candidate) => candidate.ticket_id === ticket.id,
    );
    if (payment) {
  ...
  createManagerBackupAssignment(
    ...
    if (
      employee.employment_status !== EmploymentStatuses.Active ||
      backup.employment_status !== EmploymentStatuses.Active
    ) {
      throw badRequest(
        "Inactive users cannot be selected as manager backup assignment participants",
      );
    }
  ...
  audit(
    ...
    const rows = this.store.auditLogs.filter(
      (log) => log.ticket_id === ticketId,
    );
  ...
  verifyExpenseDocument(
    ...
    if (document.verification_status === "rejected") {
      throw badRequest(
        "Rejected documents cannot be verified without a new upload",
        {
          document_id: documentId,
  ...
  private presentPaginatedTickets(
    items: ExpenseTicket[],
    page: number,
    pageSize: number,
  ): PaginatedResult<Record<string, unknown>> {
  ...
  private presentTicket(ticket: ExpenseTicket): Record<string, unknown> {
    ...
    const financeActor = ticket.finance_approver_id
      ? (this.store.users.find(
          (user) => user.id === ticket.finance_approver_id && !user.deleted_at,
        ) ?? null)
      : null;
    ...
    const governanceNotes = Array.isArray(ticket.route_snapshot.notes)
      ...
      : [];
    const expenseDocuments = this.store.expenseDocuments.filter(
      (document) => document.ticket_id === ticket.id,
    );
    return {
      ...ticket,
      ...
      finance_approver_label: financeActor
        ? `${financeActor.employee_code} - ${financeActor.full_name}`
        : ticket.finance_approver_id,
      assigned_finance_actor_user_id: ticket.finance_approver_id,
      assigned_finance_actor_label: financeActor
        ? `${financeActor.employee_code} - ${financeActor.full_name}`
        : ticket.finance_approver_id,
      ...
      finance_approval_backup_user_id:
        typeof ticket.route_snapshot.finance_approval_backup_user_id ===
        "string"
      ...
      governance_warning_codes: governanceNotes.filter(
        (note) =>
          note.startsWith("finance_") ||
          note.startsWith("primary_") ||
          note.startsWith("manager_"),
      ),
  ...
  private visibleTickets(actor: AuthUser): ExpenseTicket[] {
    return this.store.tickets.filter(
      (ticket) => !ticket.deleted_at && canReadTicket(actor, ticket),
    );
  }
  ...
  ): Array<{ bucket: string; count: number; total_amount: string }> {
    const buckets = [
  ...
  private compareFinanceQueueTickets(
    left: ExpenseTicket,
    right: ExpenseTicket,
    sort: FinanceQueueFilters["sort"],
  ): number {
    if (sort === "amount") {
  ...
  private requireRemarksForDecision(
    decision: ExpenseDecision,
    remarks?: string,
  ): void {
    const remarkRequiredDecisions: readonly string[] = [
      ExpenseDecisions.Reject,
      ExpenseDecisions.Return,
    ];
