...
import {
  assertCanManageAdminSettings,
  assertCanManageEmailTemplates,
  assertCanManageMasterData,
  assertCanManageNotificationChannels,
  assertCanManagePolicySettings,
  assertCanManageRbac,
  assertCanManageWorkflowSettings,
...
export class AdminService {
  ...
  getCompanyProfile(actor: AuthUser): CompanyProfileResponse {
    ...
    return presentCompanyProfile(
      this.repository.getCurrentCompanyProfile(this.companyIdForActor(actor)),
    );
  }

  getAdminSecuritySettings(actor: AuthUser): AdminSecuritySettingsResponse {
    assertCanManageAdminSettings(actor);
    return presentAdminSecuritySettings(
      this.repository.getAdminSecuritySettings(),
    );
  }
  ...
  updateCompanyProfile(
    ...
    assertCanManageAdminSettings(actor);
    const company = this.repository.updateCurrentCompanyProfile(
      this.companyIdForActor(actor),
      input,
    );
  ...
  updateAdminSecuritySettings(
    actor: AuthUser,
    input: AdminSecuritySettingsUpdateInput,
  ): AdminSecuritySettingsMutationResponse {
    assertCanManageAdminSettings(actor);
    ...
    return {
      settings: presentAdminSecuritySettings(settings),
  ...
  listAdminWorkflows(
    ...
    const workflows = this.repository
      ...
      .filter(
        (workflow) =>
          !moduleFilter ||
          workflow.module.toLowerCase() === moduleFilter ||
          workflow.workflow_key.toLowerCase() === moduleFilter,
      )
      .sort(
        (a, b) =>
          adminWorkflowSortOrder(a.workflow_key) -
          adminWorkflowSortOrder(b.workflow_key),
      )
    ...
    return {
      ...
      versions: Object.fromEntries(
        workflows.map((workflow) => [workflow.workflow_key, workflow.version]),
      ),
    };
  }
  ...
  listAdminPolicies(
    ...
    const policies = this.repository
      ...
      .filter(
        (policy) =>
          !moduleFilter ||
          policy.module.toLowerCase() === moduleFilter ||
          policy.policy_key.toLowerCase() === moduleFilter,
      )
      .filter((policy) => !query.active_only || policy.status === "active")
      .sort(
        (a, b) =>
          adminPolicySortOrder(a.policy_key) -
          adminPolicySortOrder(b.policy_key),
      )
    ...
    return {
      ...
      policies,
      versions: Object.fromEntries(
        policies.map((policy) => [policy.policy_key, policy.version]),
      ),
    };
  }
  ...
  listAdminEmailTemplates(
    ...
    const templates = this.repository
      ...
      .filter(
        (template) =>
          !moduleFilter ||
          template.module.toLowerCase() === moduleFilter ||
          template.template_key.toLowerCase() === moduleFilter,
      )
      .filter(
        (template) =>
          !localeFilter || template.locale.toLowerCase() === localeFilter,
      )
    ...
    return {
      ...
      templates,
      versions: Object.fromEntries(
        templates.map((template) => [template.template_key, template.version]),
      ),
    };
  }
  ...
  listAdminNotificationChannels(
    actor: AuthUser,
    query: AdminNotificationChannelsQuery,
  ): AdminNotificationChannelListResponse {
    ...
    const channels = this.repository
      ...
      .filter(
        (channel) =>
          !moduleFilter ||
          channel.module.toLowerCase() === moduleFilter ||
          channel.event_key.toLowerCase() === moduleFilter,
      )
      .filter((channel) => !query.active_only || channel.status === "active")
      .sort(
        (a, b) =>
          adminNotificationSortOrder(a.event_key) -
          adminNotificationSortOrder(b.event_key),
      )
    ...
    return {
      ...
      events: channels,
      versions: Object.fromEntries(
        channels.map((channel) => [channel.event_key, channel.version]),
      ),
  ...
  updateAdminWorkflow(
  ...
  ): AdminWorkflowMutationResponse {
    assertCanManageWorkflowSettings(actor);
    const stages = input.stages
      ? normalizeWorkflowStages(input.stages, workflowKey)
      : undefined;
    ...
    return {
      workflow: presentAdminWorkflow(workflow),
  ...
  async updateAdminPolicy(
    ...
    const config = input.config
      ...
      : undefined;
    if (config) {
      await validateAdminPolicyConfigReferences(
        this.store,
        policyKey,
        companyId,
        config,
  ...
  updateAdminEmailTemplate(
  ...
  ): AdminEmailTemplateMutationResponse {
    assertCanManageEmailTemplates(actor);
    ...
    return {
      template: presentAdminEmailTemplate(template),
  ...
  updateAdminNotificationChannels(
    actor: AuthUser,
    input: AdminNotificationChannelsUpdateInput,
  ): AdminNotificationChannelListResponse {
    assertCanManageNotificationChannels(actor);
    const normalizedChannels = input.channels.map((channel) =>
      normalizeNotificationChannelInput(channel),
    );
    ...
    const aggregateId =
      channels.find(
        (channel) => channel.event_key === normalizedChannels[0]?.event_key,
      )?.id ?? channels[0]?.id;
    if (!aggregateId) {
      throw badRequest("No notification channels are available to update.", {
        field: "channels",
      });
    }
    appendOutboxEvent(this.store, {
    ...
    return {
      ...
      events: presented,
      versions: Object.fromEntries(
        presented.map((channel) => [channel.event_key, channel.version]),
      ),
  ...
  listAdminAuditLog(
  ...
  ): Paginated<AdminAuditLogEntryResponse> {
    ...
    const rows = this.store.outbox
      ...
      .filter(
        (event) =>
          !moduleFilter ||
          moduleForAdminEvent(event).toLowerCase().includes(moduleFilter) ||
          event.event_type.toLowerCase().includes(moduleFilter),
      )
      .filter(
        (event) =>
          !query.actor_user_id ||
          actorUserIdForEvent(event) === query.actor_user_id,
      )
  ...
  listRbacRoles(
    actor: AuthUser,
    query: RbacRolesQuery,
  ): Paginated<RbacRoleResponse> {
    ...
    const filtered = this.repository
      ...
      .sort((a, b) => a.name.localeCompare(b.name));
    return page(
      filtered.map((role) => this.presentRbacRole(role)),
      query.page,
      query.page_size,
    );
  }
  ...
  listRbacPermissions(
    ...
    return {
      items: permissionCatalog().filter((permission) => {
        if (moduleFilter && permission.group.toLowerCase() !== moduleFilter)
          return false;
  ...
  updateRbacRole(
    ...
    if (current.role_key === Roles.Admin && input.status === "inactive") {
      throw conflict("Protected Admin role cannot be deactivated.", {
  ...
  replaceRbacRolePermissions(
    ...
    if (role.role_key === Roles.Admin) {
      throw conflict("Protected Admin role permissions cannot be replaced.", {
        role_key: role.role_key,
      });
    }
  ...
  listDepartments(
    ...
    const filtered = this.repository
      ...
      .filter(
        (department) => !query.active_only || department.status === "active",
      )
      .filter((department) => {
        if (!search) return true;
        return (
    ...
    return page(
      filtered.map((department) => presentDepartment(department)),
      query.page,
      query.page_size,
    );
  }
  ...
  createDepartment(
  ...
  ): MasterDataMutationResponse<DepartmentResponse> {
    assertCanManageMasterData(actor);
    const companyId = this.companyIdForActor(actor);
    this.assertValidDepartmentParent(
      companyId,
      input.parent_department_id ?? input.parent_id ?? null,
    );
    ...
    return {
      department: presentDepartment(department),
  ...
  listDesignations(
    ...
    const filtered = this.repository
      ...
      .filter(
        (designation) => !query.active_only || designation.status === "active",
      )
      .filter((designation) => {
        if (!search) return true;
        return (
          designation.title.toLowerCase().includes(search) ||
      ...
      .sort(
        (a, b) =>
          (a.level ?? 999)
            .toString()
            .localeCompare((b.level ?? 999).toString()) ||
          a.designation_code.localeCompare(b.designation_code),
      );
    return page(
      filtered.map((designation) => presentDesignation(designation)),
      query.page,
      query.page_size,
    );
  }
  ...
  createDesignation(
  ...
  ): MasterDataMutationResponse<DesignationResponse> {
    assertCanManageMasterData(actor);
    const designation = this.repository.createDesignation(
      this.companyIdForActor(actor),
      input,
    );
    appendOutboxEvent(this.store, {
    ...
    return {
      designation: presentDesignation(designation),
  ...
  listExtendedMasterData(
    ...
    query: MasterDataQuery,
  ): Paginated<ExtendedMasterDataResponse> {
    assertCanManageMasterData(actor);
    const search = query.search?.trim().toLowerCase();
    const filtered = this.repository
    ...
    return page(
      filtered.map((item) => presentExtendedMasterData(item)),
      query.page,
      query.page_size,
    );
  }
  ...
  private assertValidDepartmentParent(
    companyId: string | null,
    ...
    const parent = departments.find(
      (department) =>
        department.id === parentId && department.status === "active",
    );
    if (!parent) {
    ...
    while (cursor?.parent_department_id) {
      if (cursor.parent_department_id === currentId) {
        throw conflict("Department parent would create a hierarchy cycle.", {
          parent_id: parentId,
        });
      }
      cursor = departments.find(
        (department) => department.id === cursor?.parent_department_id,
      );
    }
  }

  private assertNoActiveUsersReference(
    companyId: string | null,
    ...
    const hasActiveReference = this.store.users.some(
      (user) =>
        ...
        (kind === "department"
          ...
          : user.designation_id === id),
    );
    if (hasActiveReference) {
      throw conflict(
        `Cannot deactivate ${kind} while active employees reference it.`,
        { id },
      );
    }
  }
  ...
  private assertKnownPermissionIds(permissionIds: readonly string[]): void {
    ...
    const unknown = permissionIds.filter(
      (permissionId) => !known.has(permissionId),
    );
    if (unknown.length) {
      throw badRequest("One or more permission IDs are not supported.", {
  ...
  private presentRbacRole(role: RbacRoleRecord): RbacRoleResponse {
    const permissionIds = this.repository
      .rolePermissionsFor(role.role_key)
    ...
    return {
      ...
      assigned_users: this.store.users.filter(
        (user) =>
          !user.deleted_at &&
          user.roles.includes(role.role_key as AuthUser["roles"][number]),
      ).length,
...
function presentCompanyProfile(
...
function presentAdminSecuritySettings(
  settings: AdminSecuritySettingsRecord,
...
function presentExtendedMasterData(
  item: AdminMasterDataItemRecord,
): ExtendedMasterDataResponse {
  return {
    id: item.id,
    master_key: item.master_key,
    key: item.master_key,
...
function presentAdminWorkflow(
  workflow: AdminWorkflowConfigRecord,
...
function presentAdminPolicy(
  ...
  return {
    ...
    derived_preview:
      policy.policy_key === "leave"
        ? leavePolicyPreview(policy.config)
        : undefined,
...
function leavePolicyPreview(
  ...
  return {
    ...
    carry_forward_cap_days:
      carryForwardCap > 0 ? carryForwardCap : "Not configured",
    negative_balance_allowed: false,
    probation_eligibility: "Not configured",
    encashment: booleanPolicyValue(config, "encashmentAllowed", false)
      ? "Configured as allowed"
      : "Not configured / future scope",
  };
}

function presentAdminEmailTemplate(
  template: AdminEmailTemplateRecord,
...
function presentAdminNotificationChannel(
  channel: AdminNotificationChannelRecord,
...
function presentAdminAuditLogEntry(
  store: MemoryDataStore,
  event: OutboxEvent,
): AdminAuditLogEntryResponse {
  const actorUserId = actorUserIdForEvent(event);
  const actor = actorUserId
    ? store.users.find((user) => user.id === actorUserId && !user.deleted_at)
    : undefined;
  return {
    id: `AL-${event.id}`,
...
function normalizeWorkflowStages(
  stages: readonly AdminWorkflowStageInput[],
  workflowKey: AdminWorkflowKey,
): AdminWorkflowStageRecord[] {
  const seen = new Set<string>();
  return stages.map((stage, index) => {
    const order = stage.order ?? index + 1;
    const id = (stage.id?.trim() || `${workflowKey}_stage_${order}`).slice(
      0,
      80,
    );
    if (seen.has(id)) {
      throw badRequest("Workflow stage IDs must be unique.", {
        field: "stages",
        id,
      });
    }
    ...
    if (!approverType || !AdminWorkflowApproverTypes.includes(approverType)) {
      throw badRequest("Workflow stage approver type is required.", {
        field: "approver_type",
      });
    }
    const approverValue = (
    ...
    if (!approverValue) {
      throw badRequest("Workflow stage approver value is required.", {
        field: "approver_value",
      });
    }
    return {
      id,
      ...
      escalate_after_days:
        stage.escalate_after_days ?? stage.escalateAfterDays ?? 0,
      mandatory_remarks_on_reject:
        stage.mandatory_remarks_on_reject ??
        stage.mandatoryRemarksOnReject ??
        true,
    };
  });
}
...
function adminNotificationSortOrder(
  eventKey: AdminNotificationEventKey,
): number {
  return AdminNotificationEventKeys.indexOf(eventKey);
}

function normalizeNotificationChannelInput(
  channel: AdminNotificationChannelInput,
) {
  const eventKey = channel.event_key ?? channel.key;
  if (!eventKey || !AdminNotificationEventKeys.includes(eventKey)) {
    throw badRequest("Notification event key is required.", {
      field: "event_key",
    });
  }
  return {
...
function maxAdminNotificationVersion(
  channels: readonly { version: number }[],
): number {
  return Math.max(1, ...channels.map((channel) => channel.version));
}

function actorUserIdForEvent(event: OutboxEvent): string | null {
  return typeof event.payload.actor_user_id === "string"
...
function moduleForAdminEvent(event: OutboxEvent): string {
  ...
  if (event.event_type.includes("notification_channels"))
...
function targetForAdminEvent(event: OutboxEvent): string {
  ...
  const candidates = [
    ...
    Array.isArray(payload.event_keys)
      ? payload.event_keys.join(", ")
      : undefined,
  ];
...
const attendanceBooleanPolicyFields = new Set([
  "allowRegularization",
  "fullDayPunchWindow",
  "autoPunchOutEnabled",
  "allowOffDayPunches",
...
function normalizeAdminPolicyConfig(
  ...
  for (const [key, value] of Object.entries(patch)) {
    if (!allowedKeys.has(key)) {
      throw badRequest("Unknown policy configuration field.", {
    ...
    if (
      ...
      attendanceBooleanPolicyFields.has(key) &&
      typeof value !== "boolean"
    ) {
      throw badRequest(
        "Attendance policy toggle values must be true or false.",
        { policy_key: policyKey, field: key },
      );
    }
    if (policyKey === "attendance" && attendanceTimePolicyFields.has(key)) {
      if (
        typeof value !== "string" ||
        !attendanceTimePolicyPattern.test(value.trim())
      ) {
        throw badRequest("Attendance punch windows must use HH:mm time.", {
          policy_key: policyKey,
          field: key,
        });
      }
    }
    const attendanceModeValues =
      policyKey === "attendance" ? attendanceModeFields[key] : undefined;
    if (
      attendanceModeValues &&
      (typeof value !== "string" ||
        !attendanceModeValues.includes(value.trim()))
    ) {
      throw badRequest("Attendance policy mode value is invalid.", {
        policy_key: policyKey,
        field: key,
      });
    }
    if (
      policyKey === "attendance" &&
      key === "effectiveGeofenceId" &&
      value !== null
    ) {
      if (typeof value !== "string" || !uuidPolicyPattern.test(value.trim())) {
        throw badRequest(
          "Attendance policy geofence reference must be a UUID.",
          { policy_key: policyKey, field: key },
        );
      }
    }
    if (typeof value === "string") {
      ...
      if (!trimmed) {
        throw badRequest("Policy text values cannot be blank.", {
          policy_key: policyKey,
          field: key,
        });
      }
...
async function validateAdminPolicyConfigReferences(
  store: MemoryDataStore,
  policyKey: AdminPolicyKey,
  companyId: string | null,
  config: Record<string, unknown>,
  ...
  if (!store.pgPool) {
    throw new Error(
      "PostgreSQL is required to validate attendance policy geofence references.",
    );
  }
