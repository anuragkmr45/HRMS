...
const attendanceMinutePolicyFields = new Set(["graceMinutes", "halfDayAfterMinutes", "autoMarkAbsentMinutes"]);
...
function normalizeAdminPolicyConfig(
  ...
  for (const [key, value] of Object.entries(patch)) {
    ...
    if (
      ...
      attendanceMinutePolicyFields.has(key) &&
      (typeof value !== "number" || !Number.isInteger(value) || value > 1440)
    ) {
      throw badRequest("Attendance minute values must be whole numbers from 0 to 1440.", {
  ...
  if (policyKey === "attendance") {
    next.attendanceMode ??= "manual_only";
    next.fallbackApprovalMode ??= "disabled";
    next.regularizationMode ??=
      next.allowRegularization === false ? "disabled" : "approval_required";
    if ("allowRegularization" in patch && !("regularizationMode" in patch)) {
      next.regularizationMode = next.allowRegularization ? "approval_required" : "disabled";
    }
    if ("regularizationMode" in patch && !("allowRegularization" in patch)) {
      next.allowRegularization = next.regularizationMode === "approval_required";
    }
    const regularizationEnabled = next.regularizationMode === "approval_required";
    if (typeof next.allowRegularization === "boolean" && next.allowRegularization !== regularizationEnabled) {
      throw badRequest("Regularization toggle and approval mode must be consistent.", {
        policy_key: policyKey,
        fields: ["allowRegularization", "regularizationMode"]
      });
    }
    if (next.attendanceMode === "manual_only" && next.fallbackApprovalMode !== "disabled") {
      throw badRequest("Manual-only attendance cannot enable a location fallback.", {
        policy_key: policyKey,
        fields: ["attendanceMode", "fallbackApprovalMode"]
      });
    }
  }
  return next;
