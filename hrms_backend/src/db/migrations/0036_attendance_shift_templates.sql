...
CREATE TABLE IF NOT EXISTS attendance.shift_instances (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL,
  employee_user_id uuid NOT NULL REFERENCES core.users(id),
  ...
  eligibility_end_at timestamptz NOT NULL,
  generation_source text NOT NULL,
  generated_at timestamptz NOT NULL DEFAULT now(),
  deleted_at timestamptz NULL,
  CONSTRAINT attendance_shift_instances_template_company_fk
    FOREIGN KEY (template_id, company_id)
    REFERENCES attendance.shift_templates (id, company_id),
  CONSTRAINT attendance_shift_instances_version_company_fk
    FOREIGN KEY (template_version_id, company_id)
    REFERENCES attendance.shift_template_versions (id, company_id),
  CONSTRAINT attendance_shift_instances_assignment_company_fk
    FOREIGN KEY (assignment_id, company_id)
    REFERENCES attendance.shift_assignments (id, company_id),
  CONSTRAINT attendance_shift_instances_generation_source_check
    CHECK (generation_source IN ('assignment', 'company_default', 'built_in_default')),
  CONSTRAINT attendance_shift_instances_schedule_check
    CHECK (
      scheduled_end_at > scheduled_start_at
      AND eligibility_start_at <= scheduled_start_at
      AND eligibility_end_at >= scheduled_end_at
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS attendance_shift_instances_employee_date_uq
  ON attendance.shift_instances (company_id, employee_user_id, work_date)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS attendance_shift_instances_template_version_idx
  ON attendance.shift_instances (company_id, template_version_id, work_date)
  WHERE deleted_at IS NULL;
...
CREATE OR REPLACE FUNCTION attendance.reject_referenced_shift_version_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF EXISTS (
    SELECT 1
    FROM attendance.shift_instances instance
    WHERE instance.company_id = OLD.company_id
      AND instance.template_version_id = OLD.id
      AND instance.deleted_at IS NULL
  ) THEN
    RAISE EXCEPTION 'attendance shift template versions referenced by generated instances are immutable'
      USING ERRCODE = '23514',
        CONSTRAINT = 'attendance_shift_versions_referenced_immutable';
  END IF;
...
DROP TRIGGER IF EXISTS attendance_shift_versions_referenced_immutable_trg
  ON attendance.shift_template_versions;
CREATE TRIGGER attendance_shift_versions_referenced_immutable_trg
  BEFORE UPDATE OR DELETE
  ON attendance.shift_template_versions
  FOR EACH ROW EXECUTE FUNCTION attendance.reject_referenced_shift_version_mutation();
